...
module.exports = cds.service.impl(function () {
    ...
    this.on('CreateClaimMDK', async (req) => {
        try{
        var client = await dbClass.createConnectionFromEnv();
        var dbConn = new dbClass(client);
        let connection = await cds.connect.to('db');
        ...
        var crHeader = JSON.parse(req.data.crHeader);
        var crItems = JSON.parse(req.data.crItems);
        var crAttachments = JSON.parse(req.data.crAttachments);
        var crEvent = JSON.parse(req.data.crEvent);
