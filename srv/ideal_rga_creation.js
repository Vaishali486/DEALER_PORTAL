...
module.exports = cds.service.impl(async function (req) {
    ...
    this.on('rgaProcessMdk', async (req) => {      
        try {
            ...
            var rgHeader = JSON.parse(req.data.rgHeader);
            var rgItems = JSON.parse(req.data.rgItems);
            var rgEvent = JSON.parse(req.data.rgEvent);
            ...
            if (sAction === "CREATE") {
                ...
                var sp = await dbconn.loadProcedurePromisified(hdbext, null, 'RGA_CREATE');
                output = await dbconn.callProcedurePromisified(sp, [sAction, appType, checkApprover[0].LEVEL, checkApprover[0].ROLE_CODE, rgHeader, rgItems, rgEvent]);
                var Result = output.outputScalar;
                ...
                ErrLogReqNo = rgaReqNo;
                    ...
                    // return rgaReqNo;
                    return output.outputScalar.OUT_SUCCESS;
                ...
                isEmailNotificationEnabled = await lib_email.isiDealSettingEnabled(connection, "VM_EMAIL_NOTIFICATION");
                    ...
                    var sp = await dbconn.loadProcedurePromisified(hdbext, null, 'RGA_APPROVE_REJECT');
                    output = await dbconn.callProcedurePromisified(sp, [sAction, appType, rgaNo, null, null, rgHeader, rgItems, rgEvent, sapNo]);
                    ...
                    var sp = await dbconn.loadProcedurePromisified(hdbext, null, 'RGA_APPROVE_REJECT');
                    output = await dbconn.callProcedurePromisified(sp, [sAction, appType, rgaNo, CcheckApprover[0].LEVEL, CcheckApprover[0].ROLE_CODE, rgHeader, rgItems, rgEvent, sapNo]);
                ...
                var sp = await dbconn.loadProcedurePromisified(hdbext, null, 'RGA_APPROVE_REJECT');
                output = await dbconn.callProcedurePromisified(sp, [sAction, appType, rgaNo, getApprover[0].LEVEL, getApprover[0].ROLE_CODE, rgHeader, rgItems, rgEvent, sapNo]);
                var Result = output.outputScalar;
